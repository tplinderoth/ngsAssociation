...
//  char task[60], csave[60];
  ...
  factr = FACTR;
  pgtol = PGTOL;
  int optfail = 0; // TL - added to signify optimization failure 2/16/15
  ...
  return like; // made function return like instead of "-like" TL 1/30/2013
}
...
/* Computing 2nd power */
    ...
    mainlb_(n, m, &x[1], &l[1], &u[1], &nbd[1], f, &g[1], factr, pgtol, &wa[
	    ...
	    << 1) + 1], task, iprint, csave, &lsave[1], &isave[22], &dsave[1],
	     (ftnlen)60, (ftnlen)60, optfail); //TL mod 2/16/15
    return 0;
} /* setulb_ */
