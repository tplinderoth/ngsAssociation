...
public:
	SiteData ();
	unsigned int cov (char allele='\0') const; /* returns count of 'allele', and total coverage if no argument is provided */
	std::string id() const; /* returns _id member */
	std::string& id(); /* returns &_id */
	std::vector<seqread> rdat;
private:
	size_t depth;
	int allecount [4]; /* counts of [A, C, G, T] alleles */
...
public:
	...
	char empiricalMajor (); /* returns the most common base for site */
