...
namespace SiteStat
{
	double assoclrt1 (Pileup* pile, std::vector<treatdat>* treatment, double like [], int* status, int verb = -1); /* returns likelihood ratio for a treatment/minor-allele association, SYK method*/
	...
	double pread (int k, double qscore, int s, const char obs, const char minor); /* calculates P(X_r|X_r,true)*P(X_r,true|k,err) */
	double readProb (const char obs, const char major, const char minor, unsigned int nminor,
	...
	char unknownMinor (char major); /* return an unused minor allele */
};
